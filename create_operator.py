...
def create_operator(email, password):
    """
    運営者アカウントをデータベースに作成する関数
    """
    # --- データベース接続 ---
    ...
    # --- データベースへの挿入 ---
    try:
        users_collection.insert_one(new_operator)
        print(f"運営者アカウント '{email}' を作成しました。")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("使用法: python create_operator.py <email> <password>")
        sys.exit(1)

    email_arg = sys.argv[1]
    password_arg = sys.argv[2]

    create_operator(email_arg, password_arg)
